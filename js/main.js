...
function user_register() {
    ...
    if (mobile.length != 10) {
        document.getElementById('mobile_error').innerHTML = " Please enter valid 10 digit number.";
        return false;
    }
    if (isNaN(mobile)) {
    ...
    if (password.length < 8) {
        document.getElementById('password_error').innerHTML = " Password must be atleast 8 character.";
        return false;
    }
    if (password.search(/[0-9]/) == -1) {
        document.getElementById('password_error').innerHTML = " Password must have atleast 1 number.";
        return false;
    }
    if (password.search(/[a-z]/) == -1) {
        document.getElementById('password_error').innerHTML = " Password must have atleast 1 lowercase.";
        return false;
    }
    if (password.search(/[A-Z]/) == -1) {
        document.getElementById('password_error').innerHTML = " Password must have atleast 1 UPPERCASE.";
        return false;
    }
    if (password.search(/[!\@\#\$\%\^\&\*\(\)\_\+\.\,\;\:]/) == -1) {
        document.getElementById('password_error').innerHTML = " Password must have atleast 1 special character.";
        return false;
    }

    //  confirm password validation
    if (cpassword == "") {
        document.getElementById('cpassword_error').innerHTML = " Please enter above password.";
        return false;
    }
    ...
    if (is_error == '') {
        jQuery.ajax({
            ...
            data: 'name=' + name + '&email=' + email + '&mobile=' + mobile + '&password=' + password + '&referral_code=' + referral_code,

            success: function (result) {
                if (result == 'email_present') {
                    //jQuery('#email_error').html('Email id already present');
                    swal("Email id already present!!!", "eStore-Nepal", "error");
                }
                if (result == 'invalid_referral_code') {
                    //jQuery('#email_error').html('Invalid Referral Code');
                ...
                if (result == 'insert'){
                  ...
                  swal("Registration Successful!!!", "eStore-Nepal", "success");
...
function user_login() {
    ...
    if (is_error == '') {
        jQuery.ajax({
            ...
            success: function (result) {
                if (result == 'wrong') {
                    swal("Please enter valid login details", "eStore-Nepal", "error");
                }
                if (result == 'valid') {
                    //jQuery('.login_msg p').html('Login Successful');
                    swal("Login Successful!!!", "eStore-Nepal", "success");
