...
if(isset($_GET['id']) && $_GET['id']!=''){
	...
	$res=mysqli_query($con,"select * from coupons where id='$id'");
...
if(isset($_POST['submit'])){
	...
	$res=mysqli_query($con,"select * from coupons where coupon_code='$coupon_code'");
	$check=mysqli_num_rows($res);
	if($check>0){
		if(isset($_GET['id']) && $_GET['id']!=''){
	...
	if($msg==''){
		if(isset($_GET['id']) && $_GET['id']!=''){
			$update_sql="update coupons set coupon_code='$coupon_code',coupon_value='$coupon_value',coupon_type='$coupon_type' where id='$id'";
			mysqli_query($con,$update_sql);
		}else{
			mysqli_query($con,"insert into coupons(coupon_code,coupon_value,coupon_type,status) values('$coupon_code','$coupon_value','$coupon_type',1)");
		}
		header('location:coupon.php');
		die();
	}
